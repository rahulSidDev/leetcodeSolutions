...
the same or another computer environment. Design an algorithm to serialize and deserialize a binary tree. 
...
class Codec:
    ...
    def serialize(self, root):
        ...
        :rtype: str
        """
        def helperRecur(node):
            if node:
                vals.append(str(node.val))
                helperRecur(node.left)
                helperRecur(node.right)
            else:
                vals.append('#')
    ...
    def deserialize(self, data):
        ...
        def helperRecur():
            ...
            node.left = helperRecur()
            node.right = helperRecur()
            return node
